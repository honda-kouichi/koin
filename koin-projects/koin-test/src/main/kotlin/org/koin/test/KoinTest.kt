...
fun KoinTest.dryRun(): Unit =
    error("Please use the checkModules() function to checkModules your list of modules.")
...
inline fun <reified T : Any> KoinTest.declareMock(
    ...
    Koin.logger?.debug("[mock] declareDefinition mock for $clazz")
...
fun dumpModulePaths() {
    ...
    getKoinConfig().getKoin().instanceRegistry.pathRegistry.paths.forEach {
        Koin.logger?.info(
            "[$it]"
        )
    }
